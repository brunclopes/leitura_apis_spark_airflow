...
for acao in acoes:
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={acao}&apikey={key_alphavantage}&datatype=csv"
    r = requests.get(url)
    tabela = pd.read_csv(StringIO(r.text))
...
coluna_particao = "dt"
sink = "raw" 
path = f"cotacao_diaria_alphavantage_{empresa}"
