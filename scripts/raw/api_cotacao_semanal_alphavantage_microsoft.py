...
# Lendo API que traz as cotações semanais com base nas ações filtradas. É criado um dataframe vazio e criado
# um loop que concatena os dataframes com as informações de cada ação
compilada = pd.DataFrame()

for acao in acoes:
    url = f"https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol={acao}&apikey={key_alphavantage}&datatype=csv"
    r = requests.get(url)
    tabela = pd.read_csv(StringIO(r.text))
    tabela["symbol"] = acao
...
df = df.select(
    ...
    ,col("low")
    ,col("close")
    ,col("adjusted close").alias("adjusted_close")
    ,col("volume")
    ,col("dividend amount").alias("dividend_amount")
    ,col("symbol")
)
