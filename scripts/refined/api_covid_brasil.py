...
SELECT 
 date as data_join
,max(dt) as max_data_join
from df
group by data_join
""")

# Join com a data maxima de ingestão para remover os dados duplicados
cond_join = [df.date == df_data.data_join, df.dt == df_data.max_data_join]
df_join = df.join(df_data, on=cond_join, how="inner").drop("data_join", "max_data_join")
